...
def _simulate(state: InputState):
    ...
    house.set_heat_loss_factor(state.heat_loss_factor)
    house.set_target_temperature(state.target_temperature_c)
...
<body>
  ...
  <form method='get' class='controls'>
    ...
    <label>Target temp °C<br><input name='target_temperature_c' type='number' step='0.1' value='{state.target_temperature_c}'></label>
    <label>Heat loss factor UA<br><input name='heat_loss_factor' type='number' step='0.01' min='0.01' value='{state.heat_loss_factor}'></label>
  ...
  <div class='card'>
    ...
    <b>Target temp:</b> {state.target_temperature_c:.1f} °C |
    <b>Heat loss factor:</b> {state.heat_loss_factor:.2f} |
    ...
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            ...
            state = InputState(
                ...
                target_temperature_c=float(query.get("target_temperature_c", [21.0])[0]),
                heat_loss_factor=max(0.01, float(query.get("heat_loss_factor", [0.18])[0])),
            )
