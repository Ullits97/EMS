    ...
    for h in range(hours):
        ...
        result.append(
            HourInput(
                ...
                temperature_setpoint_c=target_temperature_c,
            )
        )
    return result
...
class EMSApp(ttk.Frame):
    def __init__(self, master: tk.Tk) -> None:
        ...
        self.target_temp_var = tk.DoubleVar(value=21.0)
        self.heat_loss_var = tk.DoubleVar(value=0.18)
        ...
        ttk.Label(controls, text="Target temp (°C)").grid(row=1, column=0, sticky="w")
        ttk.Entry(controls, textvariable=self.target_temp_var, width=8).grid(row=1, column=1, padx=4)
        ttk.Label(controls, text="Heat loss factor UA").grid(row=1, column=2, sticky="w")
        ttk.Entry(controls, textvariable=self.heat_loss_var, width=8).grid(row=1, column=3, padx=4)
        ttk.Label(controls, text="Heating source").grid(row=1, column=4, sticky="w")
        ttk.Combobox(controls, textvariable=self.source_var, values=["district", "electric"], width=10, state="readonly").grid(row=1, column=5, padx=4)
        ttk.Button(controls, text="Run simulation", command=self.run_simulation).grid(row=1, column=6, padx=8)
    ...
    def run_simulation(self) -> None:
        ...
        target_temp = float(self.target_temp_var.get())
        heat_loss = max(0.01, float(self.heat_loss_var.get()))
        source = self.source_var.get()

        house = House(
            floor_area_m2=140.0,
            ua_kw_per_k=heat_loss,
        ...
        report = simulator.run(build_hourly_inputs(hours, ev_kwh, ev_hour, target_temp))

        self.summary_label.configure(
            text=(
                f"Heating source: {heating_source.name} | Hours: {hours} | "
                f"Target temp: {target_temp:.1f}°C | UA: {heat_loss:.2f} | "
